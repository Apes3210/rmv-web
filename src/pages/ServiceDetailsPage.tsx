  ...
  if (!service) {
    return (
      <div className="mx-auto max-w-5xl px-4 py-24 text-center sm:px-6 lg:px-8">
        ...
        <p className="mt-4 text-base text-slate-600">The service you are looking for is not available. Please return to the home page and choose a service.</p>
        <div className="mt-8">
          <Link to="/" className="text-sm font-semibold text-slate-900 hover:text-slate-700">Back to home</Link>
        </div>
      </div>
    );
  }
      ...
      <div className="mx-auto max-w-6xl px-4 py-16 sm:px-6 lg:px-8">
        <div className="flex flex-col gap-6 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="mt-4 max-w-2xl text-base leading-8 text-slate-600">{service.shortDescription}</p>
          </div>
          ...
          <div className="space-y-8">
            <section className="rounded-3xl border border-slate-200 bg-white p-8 shadow-sm">
              <div className="flex flex-wrap items-center gap-4 text-slate-700">
                ...
                <p className="text-sm uppercase tracking-[0.24em] text-slate-500">{service.headline}</p>
              </div>
              <p className="mt-6 text-lg leading-8 text-slate-700">{service.capabilityDescription}</p>
              <div className="mt-8 grid gap-4 sm:grid-cols-2">
                <div className="rounded-3xl bg-slate-50 p-6">
                  <p className="text-sm font-semibold uppercase tracking-[0.2em] text-slate-500">Best for</p>
                  <p className="mt-3 text-base leading-7 text-slate-700">{service.bestFor}</p>
                </div>
                <div className="rounded-3xl bg-slate-50 p-6">
                  <p className="text-sm font-semibold uppercase tracking-[0.2em] text-slate-500">Scope</p>
                  <p className="mt-3 text-base leading-7 text-slate-700">{service.scopeNote}</p>
                </div>
              </div>
            </section>
                ...
                <CardContent className="space-y-4">
                  <div className="grid gap-3">
                    {service.tags.map((tag) => (
                      <span key={tag} className="inline-flex items-center rounded-full border border-slate-200 bg-slate-50 px-3 py-2 text-sm font-semibold text-slate-700">
                ...
                <CardContent>
                  <div className="grid gap-3">
                    {service.systems.map((system) => (
                      <div key={system} className="rounded-3xl border border-slate-200 bg-slate-50 px-4 py-4 text-sm leading-6 text-slate-700">
                        {system}
                      </div>
                    ))}
                  </div>
            ...
            <section className="rounded-3xl border border-slate-200 bg-white p-8 shadow-sm">
              <div className="flex items-center justify-between gap-4">
                <div>
                  <p className="text-sm uppercase tracking-[0.24em] text-slate-500">Project Portfolio</p>
                  <h2 className="mt-3 text-2xl font-semibold tracking-tight text-slate-900">Recent completed projects</h2>
                </div>
                <div className="inline-flex items-center gap-2 rounded-full border border-slate-200 bg-slate-50 px-4 py-2 text-xs font-semibold uppercase tracking-[0.25em] text-slate-500">
                  <Layers className="h-4 w-4" /> Service category
                </div>
              </div>
              <div className="mt-8 grid gap-4 sm:grid-cols-2">
                {service.projects.map((project) => (
                  <article key={project.title} className="overflow-hidden rounded-3xl border border-slate-200 bg-slate-50 shadow-sm">
                    <img src={project.image} alt={project.title} className="h-44 w-full object-cover" />
                    <div className="p-5">
                      <p className="text-xs font-semibold uppercase tracking-[0.24em] text-slate-500">{project.location}</p>
              ...
              <CardContent className="space-y-5">
                <div className="flex items-center gap-3 text-slate-700">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-slate-600">(+63) 917-000-0000</p>
                  </div>
                </div>
                <div className="flex items-center gap-3 text-slate-700">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-slate-600">info@rmvfabrication.app</p>
                  </div>
                </div>
                <div className="flex items-start gap-3 text-slate-700">
                  ...
                  <div>
                    ...
                    <p className="text-sm leading-6 text-slate-600">123 Fabrication Road, Industrial City, Metro Manila</p>
                  </div>
                </div>
              </CardContent>
            </Card>
            ...
            <Card className="rounded-3xl border-slate-200 bg-white p-8 shadow-sm">
              ...
              <CardContent className="space-y-4">
                <div className="rounded-3xl bg-slate-50 p-4 text-sm text-slate-600">
                  <p className="font-semibold text-slate-900">Why choose RMV?</p>
            ...
            <section className="rounded-3xl border border-slate-200 bg-white p-8 shadow-sm">
              <div className="flex items-center gap-3 text-slate-700">
                ...
                <div>
                  ...
                  <p className="text-sm text-slate-600">We take your priorities and turn them into a project plan with budget clarity.</p>
                </div>
              </div>
            </section>
          </aside>
        ...
        <div className="mt-12 rounded-3xl border border-slate-200 bg-white p-8 shadow-sm">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
              ...
              <h2 className="mt-2 text-2xl font-semibold tracking-tight text-slate-900">Explore our full service catalogue</h2>
            </div>
            <Button asChild variant="outline" className="h-12 rounded-none">
          ...
          <div className="mt-8 grid gap-4 sm:grid-cols-3">
            {SERVICE_CATALOG.filter((item) => item.id !== service.id).map((item) => (
              <Link key={item.id} to={`/services/${item.id}`} className="group rounded-3xl border border-slate-200 bg-slate-50 p-5 text-sm text-slate-700 transition hover:border-amber-500 hover:bg-white">
                <div className="flex items-center gap-3">
                  <item.icon className="h-5 w-5 text-amber-600" />
                  <p className="font-semibold text-slate-900">{item.label}</p>
                </div>
                <p className="mt-3 leading-6 text-slate-600">{item.shortDescription}</p>
              </Link>
            ))}
          </div>
        </div>
